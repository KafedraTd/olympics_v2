...
keys=['«Русская правда»','Международный союз публикации таможенных тарифов','отдельно в самостоятельной товарной позиции',
'соблюдение условий использования товаров в соответствии с таможенными процедурами',
'проведение таможенного досмотра товаров','Постановлением Правительства РФ', 'экспорт и реимпорт',
'не позднее чем за 180 календарных дней до истечения срока годности и (или) реализации','обязан принять меры по обеспечению сохранности товаров и незамедлительно сообщить об этом в ближайший таможенный орган',
'возможна без разрешения таможенного органа и без таможенного декларирования','получить заключение уполномоченного государственного органа о возможности уничтожения товаров с указанием способа и места уничтожения',
'таможенного транзита','Ведомственную интегрированную телекоммуникационную сеть ФТС РФ','использование сети Интернет для обмена информацией с информационными системами таможенных органов',
'подлежат таможенному декларированию без помещения под таможенные процедуры','после места выдачи багажа физическим лицам',
'четвертого и пятого','трех лет со дня излишней уплаты либо излишнего взыскания таможенных платежей','36','CIP']


function getTransfering(){
    var greetingName = localStorage.getItem('loginName');
    var studEvent = localStorage.getItem('evtstu1');
...
// const fs=require('fs');
// var x=JSON.parse(fs.readFileSync('test.json'))
var x=[
    {
